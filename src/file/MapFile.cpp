...
void MapFile::getModuleMapFilename(char* buffer, int bufferSize) {
	...
	//First convert the char* to wchar_t* so it can be passed in GetModuleFileName
	const char* casted = reinterpret_cast<const char*>(buffer);
	size_t wn = mbsrtowcs(NULL, &casted, 0, NULL);
	wchar_t * buf = new wchar_t[wn + 1]();
	wn = mbsrtowcs(buf, &casted, wn + 1, NULL);

	//NextCall GetModuleFileName
	len = GetModuleFileName(GetModuleHandle(0), buf, bufferSize - 1);

	//Finally convert back from wchar_t* to char*
	size_t convertedChars = 0;
	size_t  sizeInBytes = ((sizeof(buf) + 1) * 2);
	char    *ch = (char *)malloc(sizeInBytes);
	wcstombs_s(&convertedChars, ch, sizeInBytes, buf, sizeInBytes);
	buffer = ch; //Copy the array over.
